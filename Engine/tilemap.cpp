...
bool TileMap::load(const unsigned short* tiles)
{
    ...
    m_vertices.resize(TILES_PER_SCENE_X * TILES_PER_SCENE_Y * 4 * sceneCount);
    
    // load the tileset texture
    if (!m_tileset.loadFromImage(tileImage)) { return false; }

    //for (int ii = 0; ii < sceneCount; ii++)
    //{
        // populate the vertex array, with one quad per tile
        for (unsigned int i = 0; i < TILES_PER_SCENE_X; ++i)
        {
            for (unsigned int j = 0; j < TILES_PER_SCENE_Y; ++j)
            {
                // get the current tile number
                int tileNumber = tiles[i + j * TILES_PER_SCENE_X];
                ...
                sf::Vertex* quad = &m_vertices[(i + j * TILES_PER_SCENE_X) * 4];
